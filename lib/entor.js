...
function entor({
	...
	if (!env && !sharedJson) return showError({
		message: "No env provided, getEnv failed to provide the env and/or sharedEnvPath was not provided. Check if you started your app with --env=<yourEnv>", 
		forceWarningLevel: warningLevel === "none" ? "message" : warningLevel,
		color: "red"
	});
	...
	if (syncExamples) {
		...
		let showCreatedExampleFiles = false;
		...
		arrEntors.forEach( _archivo => {
			...
			if (typeof resLeeJSON === "object" && resLeeJSON !== null) {
				...
				const exampleKeys = Object.keys(resLeeJSON);
				const sameKeys = Object.keys(loadedEntor).every( _key => exampleKeys.includes(_key));
				
				const nombreArchivoExample = _archivo.replace(/^entor/, "entorExample"); // entor.<env>.json → entorExample.<env>.json
				const isExamplePresent = fs.existsSync(join(ruta, nombreArchivoExample));
				
				
				if (!sameKeys || !isExamplePresent) {
					
					exampleKeys.forEach( _key => resLeeJSON[_key] = resLeeJSON[_key] = "" ); // quito todos los valores
		...
		if (showCreatedExampleFiles) console.log("\x1b[32m%s\x1b[0m", `[entor] Example files created.`);
